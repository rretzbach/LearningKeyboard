...
import de.rretzbach.common.markovchain.MarkovChainEntry;
import de.rretzbach.common.markovchain.MarkovChainService;
import de.rretzbach.learningkeyboard.R;
import de.rretzbach.learningkeyboard.R.layout;
import de.rretzbach.learningkeyboard.R.string;
import de.rretzbach.learningkeyboard.R.xml;
import de.rretzbach.learningkeyboard.dao.DBSuggestionService;
import de.rretzbach.learningkeyboard.dao.Follower;
import de.rretzbach.learningkeyboard.dao.FollowersDAO;
import de.rretzbach.learningkeyboard.dao.MySQLiteHelper;
...
public class LearningKeyboard extends InputMethodService implements
	...
	public void onCreate() {
		...
		followersDAO = new FollowersDAO(sqLiteHelper.getWritableDatabase());
		suggestionService = new DBSuggestionService(followersDAO);
	}
	...
	private List<Follower> extractFollowers(String lastThousandChars) {
		List<Follower> followers = new ArrayList<Follower>();

		MarkovChainService service = new MarkovChainService();
		service.parseInput(lastThousandChars);
		Map<String, MarkovChainEntry> entries = service.getEntries();
		if (entries != null && !entries.isEmpty()) {
			for (Entry<String, MarkovChainEntry> entry : entries.entrySet()) {
				for (Entry<String, Integer> occurence : entry.getValue()
						.getOccurences().entrySet()) {
					followers.add(new Follower(entry.getKey(), occurence
							.getKey(), occurence.getValue()));
				}
			}
		}
		return followers;
	...
	private void commitTyped(InputConnection inputConnection) {
		if (mComposing.length() > 0) {
			commitText(inputConnection, mComposing.toString());
		}
	}

	private void commitText(InputConnection inputConnection, String text) {
		inputConnection.commitText(text, text.length());
		
		String lastSixtyChars = getCurrentInputConnection()
				.getTextBeforeCursor(60, 0).toString();
		List<Follower> followers = new ArrayList<Follower>();
		Follower follower = extractLastFollower(lastSixtyChars);
		
		if (follower != null) {
			followers.add(follower);
			followersDAO.merge(followers);
		}
	...
	protected Follower extractLastFollower(String lastSixtyChars) {
		Pattern pattern = Pattern.compile("(\\p{L}+)\\s+(\\p{L}+)\\s*\\z");
		Matcher matcher = pattern.matcher(lastSixtyChars);
		if (matcher.find()) {
			return new Follower(matcher.group(1), matcher.group(2), 1);
		}
		return null;
	}
	...
	private void updateCandidates() {
		if (!mCompletionOn) {
			if (mComposing.length() > 0) {
				...
				/*
				 * Example: Current Input (Cursor at <>): "This is so<>" Last
				 * word: is mComposing.toString(): so suggestions could be:
				 * something, sore
				 */
				String last30Chars = String.valueOf(getCurrentInputConnection()
	...
	protected String extractLastWord(String last30Chars) {
		Pattern lastWordPattern = Pattern.compile("(?<=(?:\\s|^))(\\w+)\\s+\\w+$");
		Matcher m = lastWordPattern.matcher(last30Chars);
		if (m.find()) {
			return m.group(1);
		}
		return null;
	...
	public void pickSuggestionManually(int index) {
		if (mCandidateView != null) {
			...
			if (suggestions != null && !suggestions.isEmpty()) {
				commitText(getCurrentInputConnection(), suggestions.get(index)
						+ " ");
			}
	...
	public void pickSuggestionManuallyOriginal(int index) {
